...
def transform(ec):
    ...
    join_cond = [
        F.col("var.id_wymiar_2") == F.col("sec.id_wymiar"),
    ...
    df_transform = (df_variable_section_position.alias("sec")
                    ...
                    .select(
                        F.col("var.id_wymiar_2").cast("string").alias("id_wymiar"),
                        F.col("var.id_pozycja_2").cast("string").alias("id_pozycja"),
                        F.col("sec.nazwa_wymiar").cast("string").alias("nazwa_wymiar"),
                        F.col("sec.nazwa_pozycja").cast("string").alias("nazwa_pozycja"),
                    )
                    ).distinct()
    df_transform = df_transform.withColumn("id_dim_2", F.concat(F.col("id_wymiar"), F.col("id_pozycja")).cast("string"))
