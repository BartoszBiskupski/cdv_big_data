...
def transform(ec):
    ...
    df_transform = (df_variables.alias("var")
                    ...
                    .select(
                        F.col("var.id_wymiar_3").alias("id_wymiar"),
                        F.col("var.id_pozycja_3").alias("id_pozycja"),
                        F.col("sec.nazwa_wymiar").alias("nazwa_wymiar"),
                        F.col("sec.nazwa_pozycja").alias("nazwa_pozycja"),
                    )
                    ).distinct()
    df_transform = df_transform.withColumn("id_dim_3", F.concat(F.col("id_wymiar"), F.col("id_pozycja")))
